...
func init() {
	dataconfig, err := os.Open("./middleware/ePassword.json")
	if err != nil {
		fmt.Println(err)
	}
...
func CreateVerificationEmail(uuid, name, email, path string, condition int) (bool, map[string]interface{}) {
	...
	url := "https://clinica-fortia.netlify.app/" + path + "/" + uuid
